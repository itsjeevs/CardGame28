...
public class CurrentBoard {
    ...
    public void updateBoard(Player p, Card played) {
        ...
        if (cardsPlayed.size() == 1) {
            ...
            if (gameRef.getTrump().isOpen() && played.getUniqueCardValue().equals(trumpCardRef.getUniqueCardValue())) {
                setWasCut(true);
            }
            // Not first card. Normal case. card belong to boardSuite, and is
            // bigger.
        } else if (played.getSuit() == getGameRef().getBoardSuite() && played.getRank() < holdingCard.getRank()
                && !wasCut) {
            setCurrentHolder(p);
            setHoldingCard(played);
            if (debug) System.out
                    .println("played.getSuit() == getGameRef().getBoardSuite() && played.getRank() < holdingCard.getRank()");

            // Not first card. First Cut. cardSuite != boardSuite. cardSuite =
            // trumpSuite.
        } else if (trumpCardRef.getSuit() == played.getSuit() && played.getSuit() != gameRef.getBoardSuite()
            ...
            for (Card card : trumpOwner.getMyHand().getMyCards()) {
                if (card.getUniqueCardValue().equals(trumpCardRef.getUniqueCardValue())) alreadyIncluded = true;
            }
            ...
            if (debug) System.out
                    .println("gameRef.getTrump().getTrumpCard().getSuit() == played .getSuit() && wasCut == false");

            // card = trump. Was cut Earlier this round. Trump was open. Higher
            // cut.
        } else if (trumpCardRef.getSuit() == played.getSuit() && wasCut && gameRef.getTrump().isOpen()
        ...
        debug = true;
        if (debug) {
            System.out.println("holdingCard:" + getHoldingCard().getUniqueCardValue() + "held by: "
