...
public class Player {
    ...
    public TrumpCandidate aiPlayBid() {
        ...
        /*
         * After looping through all the suites in the loop, we will now know
         * that The suite with highest number of cards(in the hand) will be
         * saved at suiteSelected Now, If we have a tie, the suite with greater
        ...
        /*
         * Select the second biggest card from the suiteSelected, to set as
         * trump.
         */

        if (suiteSelectedCards.size() == 1) cardSelected = suiteSelectedCards.get(0);
        else cardSelected = suiteSelectedCards.get(1);

        // System.out.println("\nSuiteValue: "+suiteValue);
        if (suiteValue <= 2) bidValue = 14;
        else if (suiteValue <= 3) bidValue = 15;
        else bidValue = 16;
    ...
    public Card aiPlayGame() {
        ...
        if (!this.getName().equals(game.getPlayerTurn().getName())) return new Card();
        ...
        if (game.getBoard().getCardsPlayed().size() == 0) {
            ...
            if (returnCard == null) if (debug) System.out.println("ReturnCardNull at start");
        }
        // BoardSize !=0. I am not the first player.
            ...
            System.out.println("I'm " + getName() + "@" + getTeam().getTeamName() + ",Hand now held by team: "
                    + holdingTeam.getTeamName());
            ...
            if (this.getTeam().equals(holdingTeam)) {
                ...
                if (haveRoundSuite()) {
                    ...
                    if (returnCard != null) if (debug) System.out.println("MyTeam's hand. I'll support with: "
                            + returnCard.getUniqueCardValue());
                } else {
                    // if no cards of the same suite, support with the best card
                    // from the hand
            ...
            // If my team doesn't hold the board, I should attack.
                ...
                if (wasCut) {
                    // I have Card from suite:suiteId. Must play the MinCard
                    // from suite.
                    if (debug) System.out.println("CurrentBoard was cut");
                    if (haveRoundSuite()) {
                        returnCard = getMinCardFromHand(suiteId, true);
                        if (debug) System.out.println("I have boardSuite. I'll give the lowest card: "
                                + returnCard.getUniqueCardValue());
                    }
                    // I don't have Card form suite:suiteId. Must Cut if
                    // possible.
                    else {
                        ...
                        if (trumpInHand == null) {
                            ...
                            if (debug) System.out.println("I don't have TrumpCard. I'll give lowest card: "
                                    + returnCard.getUniqueCardValue());

                        }
                        // I have a Trump Card.
                        else {
                            // my Trump Card's value is greater than the highest
                            // Trump in the CurrentBoard.
                            if (trumpInHand.getRank() < getHoldingCard().getRank()) {
                                returnCard = trumpInHand;
                                if (debug) System.out.println("I don't have boardSuite. I'll cut: "
                                        + trumpInHand.getUniqueCardValue());
                            }
                            // Or, if I ordered to open Trump in this round,
                            // I've got to play a trump.
                            if (trumpInHand.getRank() > getHoldingCard().getRank() && openedTrumpThisRound) {
                                returnCard = getMinTrump();
                                if (debug) System.out
                                        .println("I asked to open the trump. So I have to cut with this trump "
                                                + trumpInHand.getUniqueCardValue());
                            }
                            /*
                             * My highest trump is lesser than the highest Trump
                             * in the board. don't cut. Will be called only if I
                             * didn't order to open the Trump this round
                             */
                            else {
                                trumpSuiteId = game.getTrump().getTrumpCard().getSuit();
                                returnCard = getMinCardFromHand(trumpSuiteId, false);
                                if (debug) System.out.println("I do have a trump. But smaller. So not cutting "
                                        + trumpInHand.getUniqueCardValue());
                            }
                ...
                // The hand was owned by the other team by Heirarchy.
                    ...
                    if (haveRoundSuite()) {
                        ...
                        // my biggest Card of the roundSuite is lesser than
                        // holdingCard.
                        if (bestCardInHand.getRank() > getHoldingCard().getRank()) {
                            returnCard = getMinCardFromHand(suiteId, true);
                            System.out.println("My biggest won't take the board. putting in: "
                                    + returnCard.getUniqueCardValue());
                        }
                        // my biggest Card of the roundSuite is bigger than
                        // holdingCard.
                        else {
                    ...
                    // I don't have Card from suite:roundSuite. I must cut.
                        ...
                        /*
                         * Must consider the fact that I don't have any trump
                         * cards to cut with. will be returning null at
                         * returnCard. Since I can't cut, return the lowest card
                         * I have in hand.
                         */
                        if (returnCard == null) {
                            returnCard = getMinCardFromHand(trumpSuiteId, false);
                            game.getTrump().getBidOwner().getMyHand().getMyCards().add(game.getTrump().getTrumpCard());
                            if (debug) System.out.println("I don't have a trumpCard. Minimizing casuality with: "
                                    + returnCard.getUniqueCardValue());
                        } else {
                            game.setJustOpenedTrump(true);
                            if (debug) System.out
                                    .println("Cutting with trumpCard: " + returnCard.getUniqueCardValue());
                        }
                    }
                }
            }
        }
        if (returnCard == null) returnCard = getMinCardFromHand();
    ...
    private Card getTrumpCard() {
        ...
        if (game.getTrump().isOpen()) {
            ...
            if (returnCard == null) if (debug) System.out.println("*returnCard found null at pt1");
        }
        ...
        // Open Trump, cut with the biggest Trump Value from hand.
            ...
            if (debug) System.out.println("*trumpSuiteId: " + trumpSuiteId);
            returnCard = getBiggestFromHand(trumpSuiteId, true);
            if (returnCard == null) if (debug) System.out.println("*returnCard found null at pt2");
        }
        if (returnCard != null) if (debug) System.out
                .println("The board was cut. I repeat, THE BOARD WAS CUT. Trump: " + returnCard.getUniqueCardValue());
        return returnCard;
    }
    ...
    /**
     * Returns True if contains the RoundSuite. False otherwise.
     * 
     * @return
     */
    private boolean haveRoundSuite() {
        ...
        for (Card card : myHand.getMyCards()) {
            if (card.getSuit() == suiteId) return true;
        }
        return false;
    }

    /**
     * Returns the minimum card from the hand. if include:true, minCard from the
     * specific suite. if include:false, minCard from everything outside suite.
     * 
     * @param suiteId
     * @param includeSuite
     * @return
    ...
    /**
     * Returns the biggest card from the hand. if include:true, maxCard from the
     * specific suite. if include:false, maxCard from everything outside suite.
     * 
     * @param suiteId
     * @return Card/null
     */
    private Card getBiggestFromHand(int suiteId, boolean include) {
        ...
        int trumpSuite = getTrumpSuite();
        if (debug) System.out.println("suite: " + suiteId + " include: " + include + " trump: " + trumpSuite
                + "Size of Hand:" + getMyHand().getMyCards().size());
     ...
     * Eligibility: 1. Owner of the trump. or 2. Trump is already open.
     * 
     * @return
     */
    ...
    /**
     * Returns the biggest card from the hand. Will never return suite:trump, if
     * user=trumpOwner. isValidBid Also, won't return a 9. :)
     * 
     * @return
     */
    private Card getBestCardToStart() {
        ...
        if (game.getTrump().getBidOwner().equals(getName())) suite = getPrivateTrump().getSuit();

        /*
         * putting a random int value as suiteId. Since there is no suite that
         * starts with 9, if(card.getSuit()!=suite) will always be true.
         */
        else suite = 9;

        /*
         * Trying to find the highest value card in the hand. Assuming the
         * minValue =0, we will look for values bigger than that, once the hand
         * is parsed, we'll have the highest value
         */
        for (Card card : getMyHand().getMyCards()) {
        ...
        /*
         * In case all the cards in the hand are of value = 0, the function will
         * return the first card it finds that is not a trump.
         */
        if (returnCard == null) {
            for (Card card : getMyHand().getMyCards()) {
                if (card.getSuit() != suite) {
    ...
    public Map<Integer, Float> getSuiteMap() {
        ...
        // if a trump is set, the trump value should be added to the map as
        // well.
        if (trump != null) {
    ...
    public boolean equals(Object obj) {
        ...
        if (this.points == guest.getPoints() && this.name.equals(guest.getName())
                && this.myHand.equals(guest.getMyHand()) && this.team.equals(guest.getTeam())
                && this.isAI == guest.getIsAI() && this.gameReference == guest.getGameReference()
                && this.getTrump().equals(guest.getTrump())) return true;
        else return false;
    }
