...
public class Game {
    ...
    public void rotateOnce() {
        // so that the first player in the queue now- is the one next to
        // player1.
    ...
    public boolean isValidBid(int inputTrumpValue, Player p) {
        if (isFirstRoundDone()) {
            ...
            if (inputTrumpValue > getTrump().getCurrentHightestBid() && inputTrumpValue > 14 && inputTrumpValue <= 28) return true;
            else return false;
        } else {
            if (inputTrumpValue < 14) return false;
            else return true;
        }
    }
    ...
    public void bid(Player p) {
        ...
        if (!isValidBid(trumpCandidate.getBid(), p)) return;
        ...
        if (debug) System.out.println("Bid of " + trump.getCurrentHightestBid() + " by "
                + trump.getBidOwner().getName());

        // remove the card from player's hand, and place it in Trump
        // placeholder.
    ...
    public boolean isFirstRoundDone() {
        if (this.getTrump().getCurrentHightestBid() == 13) return false;
        else return true;
    ...
    public void updateProceedings() {
        ...
        if (debug) System.out.println("Total Points for Team: " + winningTeam.getTeamName() + " " + "is: "
                + totalPoints + "(" + board.getTotalPoints() + ")");
